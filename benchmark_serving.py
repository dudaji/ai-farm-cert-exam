...
from monitor_power import calculate_avg_power_usage, monitor_npu_power_usage
...
def calculate_metrics(
    ...
    for i in range(len(outputs)):
        if outputs[i].success:
            ...
            in_len = input_requests[i].prompt_len
            per_req_input_lens.append(in_len)
    ...
    try:
        ...
        per_request_csv_path = f"{RESULT_DIR}/per_request_result-{dt}.csv"

        for idx, in_len in enumerate(per_req_input_lens):
            out_len = per_req_output_lens[idx]
            lat_s = per_req_latencies[idx]
            tps = per_req_token_thput[idx]
            rec = {
                "request_index": idx,
                "input_len_tokens": int(in_len),
                "output_len_tokens": int(out_len) if isinstance(out_len, (int, float)) else None,
        ...
        # 디렉토리 생성
        os.makedirs(os.path.dirname(per_request_csv_path), exist_ok=True)
        ...
        warnings.warn(f"Failed to persist per-request metrics: {e}", stacklevel=2)
    
    try:
        if per_req_input_lens and per_req_latencies:
            _save_length_plots(
                per_req_input_lens=per_req_input_lens,
                per_req_latencies=per_req_latencies,
                per_req_token_thput=per_req_token_thput,
            )
    except Exception as e:
        warnings.warn(f"Failed to generate length-based plots: {e}", stacklevel=2)


    return metrics, actual_output_lens

def _save_length_plots(
    per_req_input_lens: list[int],
    per_req_latencies: list[float],
    per_req_token_thput: list[float],
):
    """
    입력 길이(토큰) vs 응답시간(e2e latency), 입력 길이(토큰) vs 처리량(tokens/sec) 그래프 저장.
    - 산점도(scatter) + 구간(quantile)별 평균 추세선(평균)
    - 처리량은 per-request 단위로 (input+output)/latency 를 사용
    """
    os.makedirs(RESULT_DIR, exist_ok=True)

    dt = datetime.now().isoformat()
    x = np.array(per_req_input_lens, dtype=float)
    y_lat = np.array(per_req_latencies, dtype=float) * 1000.0  # ms 단위
    y_thr = np.array(per_req_token_thput, dtype=float)         # tokens/sec

    # 산점도 1: 길이 vs latency(ms)
    plt.figure()
    plt.scatter(x, y_lat, s=10)  # 색상 지정 없음
    plt.xlabel("Input length (tokens)")
    plt.ylabel("End-to-end latency (ms)")
    plt.title("Latency vs Input Length")
    lat_path = os.path.join(RESULT_DIR, f"latency_vs_length-{dt}.png")
    plt.savefig(lat_path, bbox_inches="tight", dpi=150)
    plt.close()

    # 산점도 2: 길이 vs per-request 처리량(tokens/sec)
    plt.figure()
    plt.scatter(x, y_thr, s=10)  # 색상 지정 없음
    plt.xlabel("Input length (tokens)")
    plt.ylabel("Per-request throughput (tokens/sec)")
    plt.title("Throughput vs Input Length")
    thr_path = os.path.join(RESULT_DIR, f"throughput_vs_length-{dt}.png")
